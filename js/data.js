...
function getDefaultStats(sect) {
  const base = { hp: 200, maxHp: 200, mp: 100, maxMp: 100, atk: 30, def: 15, agi: 10, crit: 5, exp: 0, gold: 50, level: 1 };
  const s = SECTS[sect];
  if (!s) return base;
  if (s.bonus.hp)   { base.hp += s.bonus.hp;   base.maxHp += s.bonus.hp; }
  if (s.bonus.mp)   { base.mp += s.bonus.mp;   base.maxMp += s.bonus.mp; }
  if (s.bonus.atk)  base.atk  += s.bonus.atk;
  if (s.bonus.def)  base.def  += s.bonus.def;
  if (s.bonus.agi)  base.agi  += s.bonus.agi;
  if (s.bonus.crit) base.crit += s.bonus.crit;
  return base;
}
