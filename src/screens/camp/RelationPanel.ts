...
function getRelationData(): { heroines: RelationChar[]; wudang: RelationChar[]; other: RelationChar[] } {
  ...
  const heroines: RelationChar[] = [
    {
      id: 'liu_qinghan', name: '柳清寒',
      img: 'picture/Female-main/柳清寒.png',
      affection: 55,
      unlocked: p.chapter >= 1,
    },
    {
      id: 'shen_nishang', name: '沈霓裳',
      img: 'picture/Female-main/沈霓裳.png',
      affection: 15,
      unlocked: p.chapter >= 2 && p.act >= 4,
    },
    {
      id: 'zhao_qinwei', name: '趙沁微',
      img: 'picture/Female-main/趙沁微.png',
      affection: 0,
      unlocked: false,
    },
    {
      id: 'mo_jiangqing', name: '墨绐青',
  ...
  const wudang: RelationChar[] = [
    {
      id: 'zhang_xuansu', name: '张玄素',
      img: 'picture/NPC/张玄素.png',
      affection: 20,
      unlocked: p.chapter >= 2,
    },
    {
      id: 'chen_jingxu', name: '陈静虚',
      img: 'picture/NPC/陈静虚.png',
      affection: 30,
      unlocked: p.chapter >= 2 && p.act >= 1,
    },
    {
      id: 'song_zhiyuan', name: '宋知远',
      img: 'picture/NPC/宋知远.png',
      affection: 35,
      unlocked: p.chapter >= 2,
    },
    {
      id: 'gu_xiaosang', name: '顾小桑',
      img: 'picture/NPC/顾小桑.png',
      affection: 40,
      unlocked: p.chapter >= 2,
    },
    {
      id: 'lu_chengzhou', name: '陆沉舟',
      img: 'picture/NPC/陆沉舟.png',
      affection: 10,
      unlocked: p.chapter >= 2 && p.act >= 3,
    },
  ];

  const other: RelationChar[] = [
    {
      id: 'zhou_boan', name: '周伯安',
...
function renderSection(title: string, chars: RelationChar[]): string {
  const cards = chars.map(renderCharCard).join('');
  return `
    <details class="rel-section" open>
      <summary class="rel-section-title">${title}</summary>
